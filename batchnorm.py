...
class cnn_apple(torch.nn.Module):
    ...
    def __init__(self):
        ...
        self.model = nn.Sequential(
            nn.Conv2d(4, 16, 5, 1, 2),nn.ReLU(),
            nn.MaxPool2d(kernel_size=2, stride=2),#image size:50*50
            nn.BatchNorm2d(16),
            nn.Conv2d(16, 32, 5, 1, 2),
            nn.MaxPool2d(kernel_size=2, stride=2),#image size:25*25
            nn.BatchNorm2d(32),
            nn.Conv2d(32, 64, 5, 1, 2),
            nn.MaxPool2d(kernel_size=2, stride=2),#image size:12*12
            nn.BatchNorm2d(64),
            nn.Conv2d(64, 128, 5, 1, 2),
            nn.MaxPool2d(kernel_size=2, stride=2),#image size:6*6
            nn.BatchNorm2d(128),
            nn.Flatten(),
...
#This is a dictionary of hyper parameters ,which you can modify to reproduce our results.
...
#If you are out of memory of CPU, you can change the device_name from 'CPU' to 'GPU'. So that you can run the following code on GPU.
device_name='CPU'#'GPU'
if device_name=='CPU':
